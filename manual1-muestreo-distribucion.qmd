...
1. Abre QGIS 4.0 y crea un proyecto nuevo: **Proyecto ▸ Nuevo ▸ Guardar**.
...
area <- st_read("data/area_estudio.gpkg")

# Reproyectar a UTM 18S para trabajar en metros
area <- st_transform(area, 32718)

tm_shape(area) + tm_borders(lwd = 2) + tm_layout(frame = FALSE)
```
:::
...
malla <- st_sample(area, size = st_area(area) / units::set_units(25^2, m^2),
                    type = "regular") |> st_sf()
...
tm_shape(area) + tm_borders() +
  tm_shape(aleatorio) + tm_dots(size = 0.2)
```
:::
